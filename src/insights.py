...
def matches_salary_range(job, salary):
    ...
    if "min_salary" not in job or "max_salary" not in job:
        raise ValueError("Max salary and min salary should be in the job dict")

    min_salary, max_salary = job["min_salary"], job["max_salary"]

    if (type(min_salary) is not int or type(max_salary) is not int):
        raise ValueError("Max salary and min salary should be numbers")

    if min_salary > max_salary:
        raise ValueError("Max salary must be greater or equal to min salary")
